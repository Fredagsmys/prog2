...
class LinkedList:
    ...
    def _remove_all(self, x, f):
        """ Task A5:
            ...
            Return the first node in the remaining list.
        """
        if f is None:  # if input is empty list
            return None

        if f.succ is None:  # if last element in list
            return f
        if f.data == x:
            self.first = f.succ
    ...
    def insert(self, data, index=0):
        ...
        f = self.first
        if index == 0:
            first = self.Node(data,self.first)
            self.first = first
        while f:
            ind += 1
            if ind == index:
                node = self.Node(data, f.succ)
                f.succ = node
                return
...
class BST:
    ...
    def __eq__(self, other):
        ...
        for node in self:
            selflst.append(node)
        for node in other:
            otherlst.append(node)
        return selflst == otherlst
