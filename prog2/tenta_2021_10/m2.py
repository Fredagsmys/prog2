...
def term(wtok):
    ...
    while wtok.get_current() in ('*', '/', '**'):
        ...
        if op == '*':
            ...
            exp = factor(wtok)
            #if not (((exp == 0 and result in (-1, 0)) or (result == 0 and exp < 0) or (result < 0 and exp < 1 and exp > 0))):
            if not ((result == 0 and exp <= 0) or (result < 0 and exp % 1 != 0)):
                result = result ** exp
            else:
                raise EvaluationError(f"Illegal operation {result} ** {exp}")
        else:
            try:
                if op == '/':
                    result = result / factor(wtok)
...
def factor(wtok):
    if wtok.get_current() == '(':
        ...
        if wtok.get_current() != '|':
            raise SyntaxError("Unbalanced |", wtok)
        wtok.next()
...
def statement(input_buffer):
    ...
    if wtok.get_current() == 'vars':
        ...
        print('Result:', result)
        # Add vars from tempvars to variables
        for key in tempvars.keys():
            val = tempvars.get(key)
            variables[key] = val
