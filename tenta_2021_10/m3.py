...
class LinkedList:
    ...
    def __init__(self, param=None):  ### First implementation  A7
        ...
        for x in param:
            ...
            temp = temp.succ
        self.first = self.first.succ  # Remove the dummy node
    ...
    def _append(self, f, x):  ##### A5
        '''
        if f.succ is None:
            f.succ = self.Node(x, None)
            return f
        else:
            self._append(f.succ, x)
            return f'''
...
class BST:
    ...
    def is_omorph(self, other_tree):  #### A8
        ...
        return iso(self.root, other_tree.root)
    ...
    def better_merge(self, other):
        self._better_merge(other.root)

    def _better_merge(self, other):
        if other:
            self.insert(other.key)
            self._better_merge(other.left)
            self._better_merge(other.right)
...
Create new node everytime: This will have complexity O(n)
append method: This will have complexity Theta(n(n/2)^2) = Theta(n^2) 
iter through list
'''
